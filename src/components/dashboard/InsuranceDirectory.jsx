...
const SERVICE_FILTERS = ["All", "Life", "Health", "General", "Motor", "Travel"];
const ENTITY_FILTERS = ["All", "Company", "Depository", "Governing Body"];
...
export default function InsuranceDirectory() {
  ...
  const filtered = COMPANIES.filter((c) => {
    ...
    const matchService =
      serviceFilter === "All" ||
      c.serviceType.toLowerCase().startsWith(serviceFilter.toLowerCase());
    const matchEntity = entityFilter === "All" || c.entityType === entityFilter;
    return matchSearch && matchService && matchEntity;
  });
    ...
    <div className="mx-auto w-full max-w-[1200px] px-3 py-5 sm:px-4 sm:py-8">
      ...
      <div className="mb-6 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Divider */}
          <div className="hidden h-5 w-px bg-[#E4E2DB] sm:block" />
            ...
            {ENTITY_FILTERS.map((f) => (
              <button
                ...
                className={cn(
                  "rounded-full px-3 py-1 font-poppins text-[11px] font-semibold transition-colors",
                  entityFilter === f
                    ? "bg-[#2ab5b5] text-white"
                    : "border border-[#E4E2DB] bg-white text-[#6B7280] hover:border-[#2ab5b5] hover:text-[#2ab5b5]",
                )}
              >
                {f}
              </button>
      ...
      {filtered.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="font-poppins text-sm text-[#9CA3AF]">No companies match your search.</p>
        </div>
      ) : (
