...
import { getWelcomeBannerSubtitle } from "@/lib/dashboard/welcomeBanner";
...
export default function DashboardWelcomeBanner({
  ...
  const subtitle = getWelcomeBannerSubtitle(user, {
    userCity: user?.city,
    workspaceNeedsSetup,
  });
    ...
    >
      ...
      {workspaceNeedsSetup ? (
        <Link
          ...
          className="mt-4 inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-[#0A4A4A] to-[#0D6060] px-5 py-2.5 font-poppins text-sm font-semibold text-[#F59E0B] shadow-[0_4px_16px_rgba(10,74,74,0.22)] transition active:scale-[0.98]"
        >
          Setup My Space
