...
import { Check, Copy, MessageCircle, Share2, Star, X } from "lucide-react";
...
import {
  buildRecommendSubmitSharePayload,
  canUseNativeShare,
  invokeNativeShare,
} from "@/lib/social/native-share-payload";
import { recommendSubmitShareDescription } from "@/lib/social/share-copy";
import { recommendShareCopy } from "@/lib/testimonials/recommend-share-copy";
import {
  buildWhatsAppRecommendShareMessage,
...
export function RequestRecommendModal() {
  const { requestRecommendOpen, closeRequestRecommend } = useTestimonialSubmit();
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && requestRecommendOpen && advisorProfile.slug.trim()) {
      setLink(getRecommendSubmitUrl(window.location.origin, advisorProfile.slug));
    }
  }, [requestRecommendOpen, advisorProfile.slug]);

  useEffect(() => {
    if (requestRecommendOpen) {
  ...
  if (!requestRecommendOpen) return null;

  const shareMessage = link
    ? buildWhatsAppRecommendShareMessage(link, {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="request-recommend-title"
    >
      <button
        type="button"
        className="absolute inset-0 bg-background/85 backdrop-blur-md"
        onClick={closeRequestRecommend}
        aria-label="Close"
      />
      ...
      <div className="relative z-10 w-full sm:max-w-md glass-strong rounded-t-3xl sm:rounded-3xl border border-white/15 shadow-2xl animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">
        <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-white/10">
          <div>
            <h2 id="request-recommend-title" className="text-lg font-bold tracking-tight">
              Request Recommendation
            </h2>
            <p className="mt-1 text-xs text-muted-foreground">
              Share your link — clients recommend you with OTP verification
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="px-5 py-5 space-y-4">
          <div
            className={cn(
              ...
              "bg-gradient-to-br from-[oklch(0.32_0.06_65)] to-[oklch(0.22_0.04_55)]",
            )}
          >
            <div className="h-1.5 bg-gradient-to-r from-[oklch(0.85_0.16_78)] to-[oklch(0.72_0.15_55)]" />
            <div className="p-4 flex gap-3">
              <div
                className={cn(
                  "size-14 shrink-0 rounded-xl flex items-center justify-center text-lg font-bold",
                  "bg-gradient-to-br from-[oklch(0.85_0.16_78)] to-[oklch(0.72_0.15_55)] text-white",
                )}
              >
                {initials}
              </div>
              <div className="min-w-0 flex-1">
                <YvityLogo
                  size={20}
                  showWordmark
                  wordmarkClassName="text-[10px] font-bold uppercase tracking-wider text-[oklch(0.85_0.16_78)]"
                  className="gap-1.5"
                />
                <p className="text-sm font-semibold text-foreground leading-tight mt-0.5 flex items-center gap-1.5">
                  <Star className="size-3.5 fill-[oklch(0.85_0.16_78)] text-[oklch(0.85_0.16_78)]" />
                  {recommendShareCopy.ogTitle}
                </p>
                <p className="text-xs text-muted-foreground mt-1 line-clamp-2">
                  {recommendSubmitShareDescription(advisorProfile.name, advisorProfile.title)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-3">
            <Button
              ...
              className={cn(
                ...
                "bg-[oklch(0.72_0.15_55)] hover:bg-[oklch(0.76_0.16_55)] text-white",
                "shadow-lg shadow-[oklch(0.72_0.15_55/0.35)]",
              )}
